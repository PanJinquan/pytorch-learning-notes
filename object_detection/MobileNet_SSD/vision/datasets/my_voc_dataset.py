...
class Dataset:
    def __init__(self,filename, image_dir,label_dir,transform=None, target_transform=None):
        ...
        self.image_list = Dataset._read_image_ids(filename)#获取所有图像名
        print("nums:{}".format(len(self.image_list)))
    def __getitem__(self, index):
        image_id=self.image_list[index]
        txt_name=image_id+'.txt'
        image_name=image_id+'.bmp'
        image_path = os.path.join(self.image_dir,image_name)
        labels_file= os.path.join(self.label_dir,txt_name)
        boxes, labels = self._get_annotation(labels_file)
    ...
    def _get_annotation(self, labels_file):
    ...
    def _read_image(self, image_file):
        ...
        image=image_processing.read_image_gbk(image_file)
        return image
